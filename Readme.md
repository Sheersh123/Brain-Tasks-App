...
- **`deployment.yml`**: Defines the `brain-task-app` deployment, managing replicas and container updates.
- **`service.yml`**: Exposes the application to the network (LoadBalancer/NodePort).
- **Role**: Ensures the application runs reliably on the EKS cluster.

### 🛠️ `scripts`
Automation utilities for deployment and maintenance.
- **`deploy_to_eks.sh`**: A helper script to configure `kubectl` for the `demo-eks` cluster and apply the Kubernetes manifests.
...
2.  **Build (`buildspec.yml`)**:
    -   Logs into **AWS ECR**.
    -   Builds the **Docker** image from the `dockerfile`.
    -   Pushes the image to the ECR repository.
    -   Updates the `Kubernetes/deployment.yml` with the new image tag.
3.  **Deploy**:
    -   The `appspec.yml` and scripts facilitate the deployment to the **AWS EKS** cluster.
